...
//**********************************************************************
//
// constructor: creates imagen, loading it from a JPEG file
...
Imagen::Imagen(const std::string & nombreArchivo ) 
{
	...
	//cout << "loading jpeg '" << nombreArchivo << "' .... " << flush ;
	
	buf = JpegFile::JpegFileToRGB( nombreArchivo.c_str(), &w, &h ) ;
	
	if (buf == NULL){
		std::cout << "error when loading (check stderr for details)" << std::endl << std::flush ;
		exit(1);
	}
